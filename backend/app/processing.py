...
from .schemas import ArtifactRecord, GenerationKind
from .store import store


class PlaceholderArtifactEngine:
    def __init__(self) -> None:
        settings.artifacts_path.mkdir(parents=True, exist_ok=True)

    def process(self, generation_id: str) -> list[ArtifactRecord]:
        ...
        if generation.kind == GenerationKind.image:
            return self._create_image_bundle(generation_id, generation.prompt, generation.request)
        return self._create_video_bundle(generation_id, generation.prompt, generation.request)

    def _create_image_bundle(self, generation_id: str, prompt: str, request: dict) -> list[ArtifactRecord]:
        ...
        manifest_path = self._write_manifest(generation_id, request, image_path)
    ...
    def _create_video_bundle(self, generation_id: str, prompt: str, request: dict) -> list[ArtifactRecord]:
        ...
        manifest_path = self._write_manifest(generation_id, request, gif_path)
    ...
    def _write_manifest(self, generation_id: str, request: dict, primary_artifact_path: Path) -> Path:
...
class GenerationProcessor:
    def __init__(self) -> None:
        self.placeholder_engine = PlaceholderArtifactEngine()
    ...
    def process_job(self, job_id: str) -> None:
        ...
        try:
            ...
            if generation.kind == GenerationKind.image and settings.image_engine != "placeholder":
                raise NotImplementedError("Only the placeholder image engine is wired in this stage")
            if generation.kind == GenerationKind.video and settings.video_engine != "placeholder":
                raise NotImplementedError("Only the placeholder video engine is wired in this stage")

            artifacts = self.placeholder_engine.process(generation.id)
            store.mark_succeeded(job.id, artifacts, "development artifact generated")
